...
For the null hypothesis that the effect of landholding on food expenditure is the same for those individuals with or without a loan, the p-values on the model summary inform us that the interaction term (landxhadloan) is statistically indifferent from 0 (for the minimum tested confidence level of $\alpha = 0.1$). This means that there we are unable to reject the null hypothesis. We can see then in the below coefficient plot that the landxhadloan term includes 0 in its range of possible values. 
